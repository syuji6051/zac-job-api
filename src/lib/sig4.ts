...
export const setSig4AuthHeader = (config: AxiosRequestConfig) => {
  const secrets = container.get<SecretsValues>(TYPES.ASM_VALUES);
  ...
  sign(options, {
    accessKeyId: secrets.PUPPETEER_API_ACCESS_KEY,
    secretAccessKey: secrets.PUPPETEER_API_SECRET_KEY,
  });
